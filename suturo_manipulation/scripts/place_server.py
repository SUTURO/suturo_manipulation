...
class PlaceServer():
    ...
    def execute_cb(self, goal):
        ...
        if giskard_result and giskard_result.SUCCESS in giskard_result.error_codes:
            ...
            self._gripper.publish_object_in_gripper(object_frame_id, goal.goal_pose, ObjectInGripper.PLACED)

            p_temp = PoseStamped()
            p_temp.header.frame_id = "map"
            p_temp.header.stamp = rospy.Time.now()
            p_temp.pose.position = Point(hsr_transform.transform.translation.x,
